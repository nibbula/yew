...
What I've mostly been up to, is:
  ...
  Lish https://github.com/nibbula/lish

As you may know, an easy way to get the requirements is with quicklisp.

Most of the things here are subtly interdependent. And almost everything is
something that is repeating stuff now available in other libraries in
quicklisp (but perhaps not when I started). I would like to remedy that, but
it means putting things into libraries, perhaps creating whole separate
projects, or perhaps posting patches and resolving differences with other
appropriate libraries. Typical lazy construction methods.
...
Some things that are all mushed up in here right now are:
  ...
  | fatchar       | Characters with all the fixings, err, I mean attributes. |
  | opsys         | Interface to operating system functions                  |
  | test          | Test running stuff which I even fail use                 |
  | dtt           | Delimited text tables, aka CSV                           |
  ...
  | unipose       | Shabby character composition                             |
  | grout         | Simple fancy output, for commands                        |
  ...
  | tree-browser  | Tree browser, using FUI.                                 |
  | pager         | Simple file viewer                                       |
  | puca          | Very simplistic version control interface                |
  | tiny-debug    | Crappy debugger for tiny-repl                            |
  | tiny-repl     | A shabby REPL that uses tiny-rl                          |
  | tiny-rl       | Buggy line editing                                       |
  |---------------+----------------------------------------------------------|

It all used to work on Linux, OSX, and Solaris, but it probably doesn't
anymore. I'd like it to work on cygwin and non-cygwin Windows and Android, and
actually any platform with a decent CL implementation running.  It has, at
