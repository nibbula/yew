...
Most of the things here are subtly interdependent. And many things are
repeating stuff now available in other libraries in quicklisp (but perhaps
not when I started). I would like to remedy that, but it means putting
things into libraries, perhaps creating whole separate projects, or perhaps
posting patches and resolving differences with other appropriate libraries.
...
anymore. I'd like it to work on cygwin and even non-cygwin Windows and
Android, and actually any platform with a decent CL implementation running.
It has, at times, worked on SBCL, CCL, Lispworks, Allegro, CLisp, ECL, CMU.
It probably only barely works on OSX and SBCL now.
